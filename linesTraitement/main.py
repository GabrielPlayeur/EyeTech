...
milieu=540
lignes_recues=[] #forme prise pour le code (voir l'input d'entrée de Gab pour de potentiels changements) [([Xa,Ya,Xb,Yb]),([Xc,Yc,Xd,Yd])]

while True:
    while lignes_recues==oldlignes:
        oldlignes=lignes_recues #système pour faire fonctionner le coed une fois par coords
        def calcul(lignes,milieu):
            ...
            if len(lignes)==1: #cas ou il n'y a qu'une seule ligne qui apparait (on part du principe que le y faible est le y du bas)
                ...
                if lignes[0][0][1]==0 and lignes[1][0][1]==0: #on prends Ya et Yc valeurs du abs(help ya trop de variations) #cas ou les deux lignes touchent la partie du bas
